...
TEST_CASE( "Testing OutputRedirector class" )
{
  /*
    ...
    fs::path default_filepath = osm::redirout.getFilepath();

    // If the file still exists for some reason, delete it before testing.
    delete_file( default_filepath );

    fs::path test_filepath = WORKING_DIR_PATH;
    test_filepath /= default_filename;
    ...
    {
      CHECK_EQ( file_exists( osm::redirout.getFilepath() ), false );

      osm::redirout.touch();

      CHECK_EQ( file_exists( osm::redirout.getFilepath() ), true );
      CHECK_EQ( find_file( osm::redirout.getFilepath() ), default_filepath );
      CHECK_EQ( delete_file( osm::redirout.getFilepath() ), true );
      CHECK_EQ( file_exists( osm::redirout.getFilepath() ), false );
    ...
    delete_file( osm::redirout.getFilepath() );
